...
- **Claude Code CLI**, installed and logged in with your own
  subscription: `npm install -g @anthropic-ai/claude-code`, then run
  `claude` once and complete the login.

Windows only: run everything below from Git Bash or WSL.
...
```
git clone https://github.com/chifangg/Shared-reprensentation.git shared-rep-tool
cd shared-rep-tool
bun install
bun run build
