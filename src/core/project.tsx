...
export function buildChatSystemPrompt(
  ...
  return [
    ...
    "",
    "Below is the full file tree of the uploaded project. You have these project tools:",
    "  • `read_project_file(path)` — fetch a file's full contents.",
    "  • `search_project_files(pattern)` — regex search across every file; returns path:line matches. Use it to LOCATE definitions and usages before deciding which files to read.",
    ...
    "  • `run_project_tests(selector?)` — execute the project's pytest suite in an isolated sandbox and return the output. This is your verification loop.",
    "",
    "Rules:",
    ...
    "  4. Always `read_project_file` an existing file before editing it. For `edit_project_file`, `old_string` must match the file exactly (including indentation) and must be unique — include a few surrounding lines as context if needed. But NEVER read a file back after you wrote or edited it: a successful tool result already confirms the change landed, and the content is in your context. Verify behavior with `run_project_tests`, not with re-reads.",
    ...
    "  8. When your work has a test suite, VERIFY by running it: write the code promptly, call `run_project_tests`, read the failures, fix, and run again until green. Do not spend minutes proving correctness in your head first — running the tests is faster and more reliable than simulating them.",
    "  9. Do NOT call any tool outside the list above. There are no shell, weather, or flight tools — ignore memories of those from other sessions.",
    "",
